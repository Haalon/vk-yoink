...
async def main(path, **kwargs):
    ...
    async with aiohttp.ClientSession() as session:
        if kwargs['fave']:
            ...
            loader = FaveLoader(session, str(child_path))
            await loader.run()

        for domain in kwargs.get('wall', []):
            child_path = path_obj / 'walls' / domain
            child_path.mkdir(parents=True, exist_ok=True)
            loader = WallLoader(session, str(child_path), domain)
            await loader.run()

        for peer_id in kwargs.get('chat', []):
            child_path = path_obj / 'chats' / peer_id
            child_path.mkdir(parents=True, exist_ok=True)
            loader = ChatLoader(session, str(child_path), peer_id)
            await loader.run()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wall', nargs='+', help='Domain(s) of users/groups to download from',
        default=['haalonean'], metavar=("user_id", "group_id"))
